...
void RefundEscrow_Method_bad(struct RefundEscrow *self, sol_address_t sender,
                             sol_uint256_t value, sol_uint256_t blocknum,
                             sol_uint256_t timestamp, sol_bool_t paid,
                             sol_address_t origin) {
  if (((self->user___state).v) == (1)) {
    ((self->user___state).v) = (0);
  }
}
...
void run_model(void) {
  ...
  while (sol_continue()) {
    ...
    case 39: {
      smartace_log("[Calling bad() on contract_3]");
      sol_address_t sender;
      ((sender).v) = (GET_ND_RANGE(64, 4, 9, "sender"));
      ((last_sender).v) = ((sender).v);
      sol_uint256_t value;
      ((value).v) = (0);
      RefundEscrow_Method_bad(contract_3, sender, value, blocknum, timestamp,
                              paid, sender);
      smartace_log("[Call successful]");
      break;
    }
