...
contract ERC20 is ERC20Basic {
  ...
  function transferFrom(address payable from, address to, uint256 value) public returns (bool);
...
contract StandardToken is ERC20, BasicToken {
  ...
  function transferFrom(address payable _from, address _to, uint256 _value) public returns (bool) {
    require(_to != address(0));
...
contract ZCrowdsale is Ownable{
  ...
  constructor(uint256 _startTime, uint256 _endTime, uint256 _ETHtoZCOrate, address payable _wallet,
              uint256 _minTransAmount, uint256 _mintedTokensCap, address _owner) public {  
  ...
  function getTokenAmount(uint256 weiAmount) public view returns(uint256) {        
    // As ** is unsupported, hard-coded it.              
    uint256 ETHtoZweiRate = ETHtoZCOrate.mul(100);
    // Note that (1 ether) is 10^18. We scale values for SmartACE.
    // return SafeMath.div((weiAmount.mul(ETHtoZweiRate)),(1 ether));
    return weiAmount.mul(ETHtoZweiRate);
  }
...
contract ZebiCoinCrowdsale is ZCrowdsale {
  constructor(uint256 _startTime, uint256 _endTime, uint256 _rate, address payable _wallet, uint256 _minTransAmount,
              uint256 _mintedTokensCap, address _owner) public
    ZCrowdsale(_startTime, _endTime, _rate, _wallet , _minTransAmount, _mintedTokensCap, _owner) {}
}

// Removed ZebiCoinTempMgr and ZebiMainCrowdsale, as unused in properties.
