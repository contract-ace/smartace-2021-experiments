...
sol_bool_t ERC20_Method_For_Token_transferFrom(
    ...
    sol_address_t origin, sol_address_t func_user_sender,
    sol_address_t func_user_recipient, sol_uint256_t func_user_amount) {
  ERC20_Method_For_Token___transfer(
      self, sender, value, blocknum, timestamp, Init_sol_bool_t(0), origin,
      Init_sol_address_t((func_user_sender).v),
      Init_sol_address_t((func_user_recipient).v),
      Init_sol_uint256_t(((func_user_amount).v) + (1)));
  ERC20_Method_For_Token___approve(
      self, sender, value, blocknum, timestamp, Init_sol_bool_t(0), origin,
...
void run_model(void) {
  ...
  while (sol_continue()) {
    ...
    uint8_t next_call;
    (next_call) = (GET_ND_RANGE(105, 0, 15, "next_call"));
    switch (next_call) {
    case 0: {
    ...
    case 12: {
      ...
      sol_uint256_t last_sender_bal = Read_Map_1(&contract_1.user___balances, arg_sender);
      sol_uint256_t last_recip_bal = Read_Map_1(&contract_1.user___balances, arg_recipient);
      sol_uint256_t last_allowed = Read_Map_2(&contract_1.user___allowances, arg_sender, sender);
      ERC20_Method_For_Token_transferFrom(
          &(contract_1), sender, value, blocknum, timestamp, paid, sender,
          arg_sender, arg_recipient, arg_amount);
      sol_uint256_t curr_sender_bal = Read_Map_1(&contract_1.user___balances, arg_sender);
      sol_uint256_t curr_recip_bal = Read_Map_1(&contract_1.user___balances, arg_recipient);
      sol_uint256_t curr_allowed = Read_Map_2(&contract_1.user___allowances, arg_sender, sender);
      if (arg_sender.v != arg_recipient.v)
      {
        sol_assert(last_sender_bal.v == curr_sender_bal.v + arg_amount.v, "A");
        sol_assert(last_recip_bal.v == curr_recip_bal.v - arg_amount.v, "B");
        sol_assert(last_allowed.v >= arg_amount.v, "C");
        sol_assert(last_allowed.v - arg_amount.v == curr_allowed.v, "D");
      }
      smartace_log("[Call successful]");
      break;
    }
