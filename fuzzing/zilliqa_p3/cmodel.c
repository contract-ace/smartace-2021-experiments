...
void ZilliqaToken_Method_changeAdmin(struct ZilliqaToken *self,
                                     ...
                                     sol_uint256_t timestamp, sol_bool_t paid,
                                     sol_address_t origin,
                                     sol_address_t func_user_newAdmin) {
  {
    (self->user_admin).v;
...
void run_model(void) {
  ...
  while (sol_continue()) {
    ...
    uint8_t next_call;
    (next_call) = (GET_ND_RANGE(100, 0, 12, "next_call"));
    switch (next_call) {
    case 0: {
    ...
    if (last.v != contract_1.user_admin.v)
    {
      sol_assert(last_sender.v == contract_1.user_owner.v, 0);
    }
  }
}