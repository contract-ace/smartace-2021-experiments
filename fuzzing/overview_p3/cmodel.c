...
void Escrow_Method_bad(struct Escrow *self, sol_address_t sender,
                       sol_uint256_t value, sol_uint256_t blocknum,
                       sol_uint256_t timestamp, sol_bool_t paid,
                       sol_address_t origin) {
  if (((self->user_state).v) == (1)) {
    ((self->user_state).v) = (2);
  }
}
...
void run_model(void) {
  ...
  while (sol_continue()) {
    sol_on_transaction();
    sol_assert(!(called_1 && called_2), "Property.");
    ...
    case 5: {
      smartace_log("[Calling bad() on contract_2]");
      sol_address_t sender;
      ((sender).v) = (GET_ND_RANGE(10, 3, 9, "sender"));
      ((last_sender).v) = ((sender).v);
      sol_uint256_t value;
      ((value).v) = (0);
      Escrow_Method_bad(contract_2, sender, value, blocknum, timestamp, paid,
                        sender);
      smartace_log("[Call successful]");
      break;
    }
