...
void ERC20Detailed_Method_For_Token_rename(
  ...
  ((self->user___symbol).v) = ((func_user_s).v);
}
...
void run_model(void) {
  ...
  while (sol_continue()) {
    sol_uint256_t last = contract_1.user___symbol;
    sol_on_transaction();
    ...
    case 6: {
      smartace_log("[Calling rename(s) on contract_1]");
      sol_address_t sender;
      ((sender).v) = (GET_ND_RANGE(10, 2, 7, "sender"));
      ((last_sender).v) = ((sender).v);
      sol_uint256_t value;
      ((value).v) = (0);
      sol_uint256_t arg_s = Init_sol_uint256_t(GET_ND_UINT(11, 256, "s"));
