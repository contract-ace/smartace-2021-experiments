...
void run_model(void) {
  ...
  while (sol_continue()) {
    ...
    case 11: {
      ...
      sol_uint256_t prev = Read_Map_1(&contract_1.user___balances, arg_to);
      ERC20_Method_For_Melon_transfer(&(contract_1), sender, value, blocknum,
                                      timestamp, paid, sender, arg_to,
                                      arg_value);
      sol_uint256_t post = Read_Map_1(&contract_1.user___balances, arg_to);
      sol_assert(post.v >= prev.v, 0);
      smartace_log("[Call successful]");
      break;
    }
    case 12: {
