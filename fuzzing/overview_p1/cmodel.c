...
void run_model(void) {
  ...
  while (sol_continue()) {
    ...
    case 4: {
      ...
      sol_uint256_t prev_bal = contract_2->model_balance;
      sol_uint256_t prev_dep = Read_Map_1(&contract_2->user_deposits, arg_p);
      Escrow_Method_claimRefund(contract_2, sender, value, blocknum, timestamp,
                                paid, sender, arg_p);
      sol_uint256_t post_bal = contract_2->model_balance;
