...
void run_model(void) {
  ...
  sol_address_t initial = Init_sol_address_t(GET_ND_RANGE(80, 0, 8, "SampleCrowdsale:wallet"));
  {
    sol_address_t sender;
  ...
  while (sol_continue()) {
    sol_on_transaction();
    sol_assert(contract_1.user___wallet.v == initial.v, "Property (1/2).");
    sol_assert(contract_1.user___wallet.v == contract_3->user___beneficiary.v, "Property (2/2).");
