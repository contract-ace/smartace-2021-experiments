...
void run_model(void) {
  ...
  while (sol_continue()) {
    ...
    case 0: {
      ...
      prev_lead = Read_Map_1(&contract_2->user___balances, sender);
      prev_gold = Read_Map_1(&contract_3->user___balances, sender);
      Alchemist_Method_transmute(&(contract_1), sender, value, blocknum,
                                 timestamp, paid, sender, arg__mass);
      post_lead = Read_Map_1(&contract_2->user___balances, sender);
      post_gold = Read_Map_1(&contract_3->user___balances, sender);
